...
class DoubleStreamBlockFlux2(nn.Module):
    ...
    def forward(self, img: Tensor, txt: Tensor, img_mod: Tensor, txt_mod: Tensor, pe: Tensor, txt_attention_mask: Tensor = None):
        ...
        img_attn = self.img_attn(img_n, pe=pe)
        txt_attn = self.txt_attn(txt_n, pe=pe, attn_mask=txt_attention_mask)
        
        img = img + img_gate_a * img_attn
        txt = txt + txt_gate_a * txt_attn
...
class Flux2(nn.Module):
    def __init__(self, params: FluxParams):
        ...
        self.double_stream_modulation_img = nn.Sequential(nn.SiLU(), nn.Linear(self.hidden_size, 6 * self.hidden_size, bias=True))
        self.double_stream_modulation_txt = nn.Sequential(nn.SiLU(), nn.Linear(self.hidden_size, 6 * self.hidden_size, bias=True))
        self.single_stream_modulation = nn.Sequential(nn.SiLU(), nn.Linear(self.hidden_size, 3 * self.hidden_size, bias=True))
